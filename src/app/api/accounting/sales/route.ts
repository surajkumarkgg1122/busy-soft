...
async function requireAccountingSetup(db:ReturnType<typeof getAdminServices>["db"],businessId:string,date:string){
  const businessRef=db.collection("businesses").doc(businessId);
  const businessSnap=await businessRef.get();
  if(!businessSnap.exists)throw new Error("Business does not exist.");
  const business=businessSnap.data() as {setupStatus?:string;financialYearStartMonth?:number};
  if(business.setupStatus&&business.setupStatus!=="ready")throw new Error("Business accounting setup is not ready.");
  const startMonth=Number(business.financialYearStartMonth??4);
  const fy=resolveFinancialYear(date,startMonth);
  const fySnap=await businessRef.collection("financialYears").doc(fy.id).get();
  if(!fySnap.exists)throw new Error("Financial year is not initialized for this business. Complete business accounting setup before posting a sale.");
  return fy.id;
}
...
export async function POST(request:Request){
  try{
    ...
    if(!authHeader.startsWith("Bearer "))return jsonError("Authentication is required.",401);
    const token=await auth.verifyIdToken(authHeader.slice(7));
    const body=await request.json() as Record<string,unknown>;
    const businessId=String(body.businessId??"");
    if(!businessId)return jsonError("Business ID is required.",400);
    const businessRef=db.collection("businesses").doc(businessId),membershipSnap=await businessRef.collection("members").doc(token.uid).get();
    if(!membershipSnap.exists)return jsonError("You are not a member of this business.",403);
    const membership=membershipSnap.data() as {status?:string;role?:string;permissions?:Record<string,unknown>};
    if(membership.status!=="active")return jsonError("Your business membership is not active.",403);
    const salesPermissions=(membership.permissions?.sales??{}) as Record<string,unknown>;
    if(!(membership.role==="owner"||membership.role==="admin"||salesPermissions.create===true))return jsonError("Permission denied: SALE_CREATE.",403);
    const date=String(body.date??"");
    const financialYearId=await requireAccountingSetup(db,businessId,date);
    const paymentMode=body.paymentMode as "cash"|"bank"|"credit";
    if(!["cash","bank","credit"].includes(paymentMode))return jsonError("Invalid payment mode.",400);
    const selectedBankAccountId=typeof body.bankAccountId==="string"&&body.bankAccountId?body.bankAccountId:undefined;
    let bankAccountName:string|undefined;
    let bankLedgerAccountId:string|undefined;
    if(paymentMode==="bank"&&Number(body.paidAmount??0)>0){
      if(!selectedBankAccountId)return jsonError("Receiving bank account is required for online/bank payment.",400);
      const bankSnap=await businessRef.collection("bankAccounts").doc(selectedBankAccountId).get();
      if(!bankSnap.exists)return jsonError("Selected receiving bank account does not exist.",400);
      const bank=bankSnap.data() as {displayName?:string;bankName?:string;ledgerAccountId?:string;kind?:string;status?:string};
      if(bank.status!=="active"||bank.kind!=="bank")return jsonError("Selected receiving account is not an active bank account.",400);
      bankLedgerAccountId=String(bank.ledgerAccountId??"");
      if(!bankLedgerAccountId)return jsonError("Selected bank account is not linked to an accounting ledger account.",400);
      const ledgerSnap=await businessRef.collection("accounts").doc(bankLedgerAccountId).get();
      if(!ledgerSnap.exists)return jsonError("The selected bank ledger account does not exist.",400);
      const ledger=ledgerSnap.data() as {type?:string;active?:boolean};
      if(ledger.active===false||ledger.type!=="asset")return jsonError("The selected bank ledger account is not an active asset account.",400);
