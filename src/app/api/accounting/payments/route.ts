...
import { executePayment, executeReceipt } from "@/application/core";
...
export async function POST(request:Request){
  try{
    ...
    if(!/^\d{4}-\d{2}-\d{2}$/.test(date))throw new ValidationError("Payment date must be YYYY-MM-DD.");if(!customerId)throw new ValidationError("Customer is required.");if(!["in","out"].includes(direction))throw new ValidationError("Invalid payment direction.");if(!Number.isSafeInteger(amountMinor)||amountMinor<=0)throw new ValidationError("Payment amount must be a positive integer minor-unit amount.");if(idempotencyKey.length<16||idempotencyKey.length>128)throw new ValidationError("Idempotency key must be between 16 and 128 characters.");
    ...
    const repo=createAdminAccountingRepository(businessId);const deps={ids:{next:(prefix:string)=>`${prefix}-${crypto.randomUUID()}`},clock:{now:()=>new Date().toISOString()}};const context={businessId,userId,financialYearId:fyId,idempotencyKey,permissions:[direction==="in"?"RECEIPT_CREATE":"PAYMENT_CREATE"] as ["RECEIPT_CREATE"|"PAYMENT_CREATE"]};const input={date,partyId:customerId,amount:amountMinor,mode:paymentMethod.toLowerCase()==="cash"?"cash":"bank",accountMap:{party:DEFAULT_PARTY_ACCOUNT,cash:DEFAULT_CASH_ACCOUNT,bank:bankAccount},narration:[paymentMethod,note].filter(Boolean).join(" · ")||undefined};
    const result=direction==="in"?await executeReceipt({repo,ids:deps.ids,clock:deps.clock},context,input):await executePayment({repo,ids:deps.ids,clock:deps.clock},context,input);
    return NextResponse.json({success:true,result:{voucher:result.value.voucher,customer:{id:customerId,name:String(customerSnap.data()?.name??"")},accountName:paymentMethod.toLowerCase()==="cash"?cashName:bankName}});
  }catch(error){const message=error instanceof Error?error.message:"Could not post payment.";const status=/permission|member/i.test(message)?403:/AUTH|token|credential/i.test(message)?401:/conflict|duplicate|idempotency/i.test(message)?409:/financial year|required|date|amount|customer|account/i.test(message)?400:500;return jsonError(message,status);}
}
