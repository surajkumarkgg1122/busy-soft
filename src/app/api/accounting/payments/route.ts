...
async function authorize(request:Request,businessId:string,action:"view"|"create"){
  const {auth,db}=getAdminServices();const header=request.headers.get("authorization")??"";
  ...
  const businessRef=db.collection("businesses").doc(businessId);const membershipSnap=await businessRef.collection("members").doc(token.uid).get();
  if(!membershipSnap.exists)throw new Error("NOT_MEMBER");const membership=membershipSnap.data() as Membership;
  if(membership.status!=="active")throw new Error("INACTIVE_MEMBER");const permissions=(membership.permissions?.payments??{}) as Record<string,unknown>;const legacy=membership.permissions?.payments===true;
  if(!(membership.role==="owner"||membership.role==="admin"||permissions[action]===true||legacy))throw new Error("PERMISSION_DENIED");
  return{db,businessRef,userId:token.uid};
}

async function normalizeAccountName(db:AdminDb,businessId:string,accountId:string){
  const snap=await db.collection("businesses").doc(businessId).collection("accounts").doc(accountId).get();
  if(!snap.exists)throw new ValidationError("Selected payment account does not exist.");
  const account=snap.data() as {name?:string;type?:string;active?:boolean};
  if(account.active===false)throw new ValidationError("Selected payment account is inactive.");
  if(account.type!=="asset")throw new ValidationError("Payment account must be an asset account.");
  return String(account.name??accountId);
}
...
export async function GET(request:Request){
  try{
    ...
    const customerMap=new Map(customersSnap.docs.map(doc=>[doc.id,String(doc.data().name??"")]));const voucherLinesSnap=await businessRef.collection("voucherLines").get();
...
export async function POST(request:Request){
  try{
    ...
    if(!/^\d{4}-\d{2}-\d{2}$/.test(date))throw new ValidationError("Payment date must be YYYY-MM-DD.");if(!customerId)throw new ValidationError("Customer is required.");if(!["in","out"].includes(direction))throw new ValidationError("Invalid payment direction.");if(!Number.isSafeInteger(amountMinor)||amountMinor<=0)throw new ValidationError("Payment amount must be a positive integer minor-unit amount.");
    ...
    const repo=createAdminAccountingRepository(businessId);const deps={ids:{next:(prefix:string)=>`${prefix}-${crypto.randomUUID()}`},clock:{now:()=>new Date().toISOString()}};const idempotencyKey=String(body.idempotencyKey??`payment-${businessId}-${fyId}-${crypto.randomUUID()}`);const accountMap={party:DEFAULT_PARTY_ACCOUNT,cash:DEFAULT_CASH_ACCOUNT,bank:bankAccount};
    const result=direction==="in"?await postReceipt(repo,{businessId,financialYearId:fyId,date,userId,partyId:customerId,amount:amountMinor,mode:paymentMethod.toLowerCase()==="cash"?"cash":"bank",accountMap},deps):await postPayment(repo,{businessId,financialYearId:fyId,date,userId,partyId:customerId,amount:amountMinor,mode:paymentMethod.toLowerCase()==="cash"?"cash":"bank",accountId:DEFAULT_PARTY_ACCOUNT,accountMap},deps);
    const documentId=`${direction}:${result.voucher.id}`;const paymentPayload={businessId,financialYearId:fyId,paymentId:documentId,voucherId:result.voucher.id,voucherNumber:result.voucher.voucherNumber,customerId,customerName:String(customerSnap.data()?.name??""),direction,amountMinor,method:paymentMethod,accountName:paymentMethod.toLowerCase()==="cash"?cashName:bankName,postedAt:result.voucher.createdAt,idempotencyKey};
