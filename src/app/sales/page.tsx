...
export default function SalesPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [c, i, s] = await Promise.all([getDocs(collection(business, "customers")), getDocs(collection(business, "items")), getDocs(collection(business, "sales"))]);
  ...
  function printInvoice() {
    const label = selectedCustomer?.name || (customerId ? "Customer" : "Cash Sale"); const rows = validLines.map(l => `<tr><td>${l.name}</td><td>${l.quantity}</td><td>${money(l.price)}</td><td>${l.discountPercent}%</td><td>${l.taxRate}%</td><td>${money(lineNet(l) + lineNet(l) * l.taxRate / 100)}</td></tr>`).join("");
  ...
  async function shareInvoice() { const text = `Sales Invoice #${invoiceNumber}\n${selectedCustomer?.name || "Cash Sale"}\nTotal: ${money(total)}\nReceived: ${money(receivedAmount)}\nBalance: ${money(balance)}`; if (navigator.share) await navigator.share({ title: `Invoice #${invoiceNumber}`, text }); else await navigator.clipboard?.writeText(text); }

  const field = "h-9 rounded border border-[#cbd1d8] bg-white px-3 text-sm text-[#263442] outline-none focus:border-[#1787f2]";
  ...
  return <AuthGate><div className="min-h-screen bg-[#f1f3f5]"><div className="flex min-h-screen"><Sidebar/><main className="min-w-0 flex-1"><TopNav/>
    {!canView ? <div className="p-6"><div className="rounded border border-red-200 bg-red-50 p-8 text-center"><h1 className="text-lg font-semibold text-red-700">Access denied</h1><p className="mt-1 text-sm text-red-600">You do not have permission to view sales.</p></div></div> : showList ? <div className="p-4 sm:p-6"><div className="mb-4 flex items-center justify-between"><div><h1 className="text-2xl font-bold text-[#243244]">Sales</h1><p className="text-sm text-[#737d88]">Sales invoices</p></div>{canCreate && <button onClick={newInvoice} className="rounded bg-[#1787f2] px-5 py-2.5 text-sm font-semibold text-white">+ Add Sale</button>}</div><div className="overflow-hidden rounded border border-[#d5d9de] bg-white"><div className="flex items-center border-b bg-[#f7f8f9] p-3"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search transactions" className="h-9 w-full max-w-md rounded border border-[#cbd1d8] px-3 text-sm outline-none focus:border-[#1787f2]"/></div><div className="overflow-x-auto"><table className="min-w-[1000px] w-full text-sm"><thead><tr className="bg-[#eef0f2] text-left text-xs font-semibold uppercase text-[#596572]"><th className="px-3 py-2">Date</th><th className="px-3 py-2">Ref No.</th><th className="px-3 py-2">Party</th><th className="px-3 py-2">Type</th><th className="px-3 py-2 text-right">Total</th><th className="px-3 py-2 text-right">Received</th><th className="px-3 py-2 text-right">Balance</th><th className="px-3 py-2">Status</th><th className="px-3 py-2 text-right">Actions</th></tr></thead><tbody>{loading ? <tr><td colSpan={9} className="p-12 text-center text-[#7b8490]">Loading…</td></tr> : filteredSales.length === 0 ? <tr><td colSpan={9} className="p-12 text-center text-[#7b8490]">No sales found.</td></tr> : filteredSales.map(s => <tr key={s.id} className="border-t hover:bg-[#f8fbff]"><td className="px-3 py-2.5">{s.invoiceDate || "—"}</td><td className="px-3 py-2.5 font-semibold">{s.invoiceNumber || "—"}</td><td className="px-3 py-2.5">{partyName(s)}</td><td className="px-3 py-2.5">Sale</td><td className="px-3 py-2.5 text-right">{money(Number(s.total || 0))}</td><td className="px-3 py-2.5 text-right">{money(Number(s.paid || 0))}</td><td className="px-3 py-2.5 text-right">{money(Number(s.balance || 0))}</td><td className="px-3 py-2.5">{s.status || "—"}</td><td className="px-3 py-2.5"><div className="flex justify-end gap-1"><button onClick={() => openSale(s, false)} className="rounded border px-2.5 py-1 text-xs text-[#465361] hover:bg-[#f1f3f5]">View</button>{canEdit && <button onClick={() => openSale(s, true)} className="rounded border border-[#1787f2] px-2.5 py-1 text-xs text-[#1787f2] hover:bg-[#eef7ff]">Edit</button>}{canDelete && <button onClick={() => deleteSale(s)} className="rounded border border-red-200 px-2.5 py-1 text-xs text-red-600 hover:bg-red-50">Delete</button>}</div></td></tr>)}</tbody></table></div></div></div> : <div className="min-h-[calc(100vh-64px)] bg-white"><div className="border-b bg-white px-4 py-3 sm:px-6"><div className="flex items-center gap-3"><button onClick={() => { setShowList(true); setSelectedSale(null); setEditing(false); }} className="text-sm text-[#1787f2]">← Sales</button><div className="h-5 w-px bg-[#d6dbe0]"/><h1 className="text-lg font-semibold text-[#243244]">Sale</h1><div className="ml-1 flex rounded-full bg-[#dcecff] p-1 text-sm"><button onClick={() => !readOnly && setPaymentType("Credit")} className={`rounded-full px-4 py-1 ${paymentType === "Credit" ? "bg-[#4285f4] text-white" : "text-[#243244]"}`}>Credit</button><button onClick={() => !readOnly && setPaymentType("Cash")} className={`rounded-full px-4 py-1 ${paymentType === "Cash" ? "bg-[#4285f4] text-white" : "text-[#243244]"}`}>Cash</button></div><div className="ml-auto flex gap-2">{selectedSale && canEdit && !editing && <button onClick={() => setEditing(true)} className="rounded border border-[#1787f2] px-4 py-1.5 text-sm text-[#1787f2]">Edit</button>}{selectedSale && canDelete && !editing && <button onClick={() => deleteSale(selectedSale)} className="rounded border border-red-200 px-4 py-1.5 text-sm text-red-600">Delete</button>}</div></div></div>
      <div className="p-4 sm:p-6"><div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_330px]"><div><div className="grid grid-cols-1 gap-4 md:grid-cols-[230px_1fr]"><div><label className="mb-1 block text-xs font-semibold text-[#6f7882]">Customer <span className="text-red-500">*</span></label><select value={customerId} onChange={e => setCustomerId(e.target.value)} disabled={readOnly} className={`w-full ${field} ${disabledField}`}><option value="">{paymentType === "Cash" ? "Cash Sale" : "Select customer"}</option>{customers.filter(c => c.status !== "inactive").map(c => <option key={c.id} value={c.id}>{c.name || "Unnamed Customer"}</option>)}</select><div className="mt-1 text-xs text-red-500">BAL: {money(customerBalance)}</div><textarea value={selectedCustomer?.address || ""} readOnly placeholder="Billing Address" className="mt-4 h-24 w-full resize-none rounded border border-[#cbd1d8] bg-white p-3 text-sm text-[#66717c] outline-none"/></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:grid-cols-1"><div><label className="mb-1 block text-xs font-semibold text-[#7a838c]">Invoice Number</label><input value={invoiceNumber} onChange={e => setInvoiceNumber(e.target.value.replace(/\D/g, ""))} disabled={readOnly} className={`w-full ${field} ${disabledField}`} /></div><div><label className="mb-1 block text-xs font-semibold text-[#7a838c]">Invoice Date</label><input type="date" value={invoiceDate} onChange={e => setInvoiceDate(e.target.value)} disabled={readOnly} className={`w-full ${field} ${disabledField}`} /></div><div><label className="mb-1 block text-xs font-semibold text-[#7a838c]">State of Supply</label><input value={stateOfSupply} onChange={e => setStateOfSupply(e.target.value)} disabled={readOnly} placeholder="Select" className={`w-full ${field} ${disabledField}`} /></div></div></div>
        <div className="mt-8 overflow-x-auto border border-[#cfd5db]"><table className="min-w-[900px] w-full text-sm"><thead><tr className="bg-[#f3f4f5] text-left text-xs font-semibold text-[#4d5965]"><th className="w-10 border-r px-2 py-2">#</th><th className="border-r px-2 py-2">ITEM</th><th className="w-20 border-r px-2 py-2">QTY</th><th className="w-20 border-r px-2 py-2">UNIT</th><th className="w-32 border-r px-2 py-2 text-right">PRICE/UNIT<br/><span className="font-normal">Without Tax</span></th><th className="w-28 border-r px-2 py-2 text-center">DISCOUNT<br/><span className="font-normal">%</span></th><th className="w-28 border-r px-2 py-2">TAX</th><th className="w-28 px-2 py-2 text-right">AMOUNT</th><th className="w-10 px-2"></th></tr></thead><tbody>{lines.map((line, index) => <tr key={`${index}-${line.itemId}`} className="border-t bg-[#fafbfc]"><td className="border-r px-2 py-2 text-center text-[#7c858f]">{index + 1}</td><td className="border-r px-2 py-1.5"><select value={line.itemId} onChange={e => updateLine(index, "itemId", e.target.value)} disabled={readOnly} className={`w-full border-0 bg-transparent text-sm outline-none ${disabledField}`}><option value="">Select item</option>{items.map(item => <option key={item.id} value={item.id}>{item.name || "Unnamed Item"}</option>)}</select></td><td className="border-r px-1"><input type="number" min="1" value={line.quantity} onChange={e => updateLine(index, "quantity", e.target.value)} disabled={readOnly} className={`w-full border-0 bg-transparent px-2 text-right outline-none ${disabledField}`} /></td><td className="border-r px-2 text-center text-xs">{line.unit}</td><td className="border-r px-1"><input type="number" min="0" value={line.price} onChange={e => updateLine(index, "price", e.target.value)} disabled={readOnly} className={`w-full border-0 bg-transparent px-2 text-right outline-none ${disabledField}`} /></td><td className="border-r px-1"><input type="number" min="0" max="100" value={line.discountPercent} onChange={e => updateLine(index, "discountPercent", e.target.value)} disabled={readOnly} className={`w-full border-0 bg-transparent px-2 text-right outline-none ${disabledField}`} /></td><td className="border-r px-2 text-center">{line.taxRate ? `GST@${line.taxRate}%` : "Select"}</td><td className="px-2 text-right font-medium">{money(lineNet(line) + lineNet(line) * line.taxRate / 100)}</td><td className="px-1 text-center">{(!selectedSale || editing) && <button onClick={() => removeRow(index)} className="text-red-500">×</button>}</td></tr>)}</tbody><tfoot><tr className="border-t bg-white"><td colSpan={2} className="px-2 py-2">{(!selectedSale || editing) && <button onClick={addRow} className="rounded border border-[#1787f2] px-3 py-1 text-xs font-semibold text-[#1787f2]">ADD ROW</button>}</td><td className="px-2 py-2 text-right font-semibold">{validLines.reduce((s,l) => s + l.quantity, 0)}</td><td colSpan={2}></td><td className="px-2 py-2 text-right font-semibold">{money(discount)}</td><td className="px-2 py-2 text-right font-semibold">{money(tax)}</td><td className="px-2 py-2 text-right font-semibold">{money(total)}</td><td></td></tr></tfoot></table></div></div>
        <div className="lg:pt-28"><div className="rounded border border-[#d5d9de] bg-[#fafafa] p-4"><label className="mb-1 block text-xs font-semibold text-[#6f7882]">Payment Type</label><select value={paymentType} onChange={e => setPaymentType(e.target.value as "Credit" | "Cash")} disabled={readOnly} className={`w-full ${field} ${disabledField}`}><option>Cash</option><option>Credit</option></select><div className="mt-5 flex items-center justify-between text-sm"><label className="flex items-center gap-2"><input type="checkbox" checked={roundOff} onChange={e => setRoundOff(e.target.checked)} disabled={readOnly}/><span>Round Off</span></label><span>{money(roundAmount)}</span></div><div className="mt-3 flex items-center justify-between"><span className="text-sm font-semibold">Total</span><input value={money(total)} readOnly className="h-9 w-40 rounded border border-[#cbd1d8] bg-white px-3 text-right text-sm"/></div><div className="mt-3 flex items-center justify-between"><span className="text-sm">Received</span><input type="number" min="0" value={paymentType === "Cash" ? total : received} onChange={e => setReceived(e.target.value)} disabled={paymentType === "Cash" || readOnly} className={`h-9 w-40 ${field} ${disabledField} text-right`}/></div><div className="mt-3 flex items-center justify-between"><span className="text-sm font-semibold">Balance</span><span className="font-semibold text-[#1787f2]">{money(balance)}</span></div></div><textarea value={note} onChange={e => setNote(e.target.value)} disabled={readOnly} placeholder="Add description" className={`mt-4 h-24 w-full resize-none ${field} ${disabledField}`}/></div></div></div>
      {error && <div className="mx-4 mb-2 rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 sm:mx-6">{error}</div>}
      <div className="sticky bottom-0 border-t bg-white px-4 py-3 sm:px-6"><div className="flex justify-end gap-2"><button onClick={shareInvoice} className="rounded border border-[#1787f2] bg-white px-5 py-2 text-sm text-[#1787f2]">Share</button><button onClick={printInvoice} className="rounded border border-[#1787f2] bg-white px-5 py-2 text-sm text-[#1787f2]">Print</button>{(!selectedSale || editing) ? <button onClick={saveInvoice} disabled={saving || (selectedSale ? !canEdit : !canCreate)} className="rounded bg-[#1787f2] px-7 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? "Saving…" : selectedSale ? "Update" : "Save"}</button> : <button onClick={() => { setShowList(true); setSelectedSale(null); setEditing(false); }} className="rounded bg-[#1787f2] px-7 py-2 text-sm font-semibold text-white">Close</button>}</div></div>
    </div>}
