...
export default function SalesPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setError("Could not load sales. Check Firestore rules and your business membership.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const saveInvoice = async () => {
    if (!firestoreDb || !activeBusinessId) {
      setError("Select a business before creating a sale.");
      return;
    }
    ...
    for (const line of validLines) {
      ...
      if (!item) {
        setError("One of the selected items is no longer available.");
        return;
      }
      if (line.quantity > Number(item.stock || 0)) {
        setError(`${item.name || "Item"} has only ${Number(item.stock || 0)} ${item.unit || "units"} available.`);
        return;
      }
    }
    ...
    try {
      ...
      await runTransaction(firestoreDb, async (transaction) => {
        ...
        if (customerId) {
          customerRef = doc(businessRef, "customers", customerId);
        ...
        if (customerRef && customerSnapshot?.exists() && balance > 0) {
          transaction.update(customerRef, { balance: currentCustomerBalance + balance, updatedAt: serverTimestamp() });
        }
        if (customerRef && customerSnapshot?.exists() && balance === 0 && receivedAmount > 0) {
          transaction.update(customerRef, { updatedAt: serverTimestamp() });
        }
        ...
        <div className="mx-auto max-w-[1450px]">
          {mode === "list" ? <>
            <section className="mb-7 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
              <div><p className="text-sm font-semibold text-[#4f46e5]">Sales workspace</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-[#182230]">Sales</h1><p className="mt-2 max-w-xl text-sm text-[#667085]">Create invoices, track payments, and keep stock and customer balances connected.</p></div>
              <button type="button" onClick={openNew} disabled={!activeBusinessId} className="inline-flex h-10 items-center justify-center rounded-xl bg-[#4f46e5] px-5 text-sm font-semibold text-white shadow-[0_8px_18px_rgba(79,70,229,.22)] hover:bg-[#4338ca] disabled:opacity-50">+ New invoice</button>
            </section>
            ...
            <div className="grid gap-6 p-6 xl:grid-cols-[minmax(0,1fr)_340px]">
              <div><div className="grid gap-4 rounded-2xl border border-[#eaecf0] bg-[#fbfaf9] p-5 md:grid-cols-3"><label className="text-sm font-semibold text-[#344054]">Customer<select value={customerId} onChange={(event) => setCustomerId(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-[#d0d5dd] bg-white px-3 font-normal outline-none focus:border-[#4f46e5]"><option value="">Walk-in customer</option>{customers.filter((customer) => (customer.status || "Active").toLowerCase() === "active").map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label><label className="text-sm font-semibold text-[#344054]">Invoice date<input type="date" value={invoiceDate} onChange={(event) => setInvoiceDate(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-[#d0d5dd] bg-white px-3 font-normal outline-none focus:border-[#4f46e5]" /></label><div className="rounded-xl bg-white p-3 text-sm"><p className="text-xs font-medium uppercase text-[#98a2b3]">Customer balance</p><p className="mt-2 font-bold text-[#182230]">{money(Number(selectedCustomer?.balance || 0))}</p></div></div>
                <div className="mt-6 rounded-2xl border border-[#eaecf0] p-5"><div className="mb-5 flex items-center justify-between"><div><h2 className="font-bold text-[#182230]">Invoice items</h2><p className="mt-1 text-sm text-[#667085]">Stock is checked again at save time.</p></div><button type="button" onClick={() => setLines((current) => [...current, { ...emptyLine }])} className="rounded-xl border border-[#c7c3ff] px-3 py-2 text-sm font-semibold text-[#4f46e5]">+ Add line</button></div><div className="space-y-3">{lines.map((line, index) => <div key={index} className="grid gap-3 rounded-xl border border-[#eaecf0] p-4 lg:grid-cols-[minmax(220px,2fr)_90px_110px_110px_110px_36px]"><select value={line.itemId} onChange={(event) => updateLine(index, "itemId", event.target.value)} className="h-10 rounded-lg border border-[#d0d5dd] bg-white px-3 text-sm"><option value="">Select item</option>{items.map((item) => <option key={item.id} value={item.id}>{item.name} · {Number(item.stock || 0)} {item.unit || "units"} left</option>)}</select><input title="Quantity" type="number" min="1" value={line.quantity} onChange={(event) => updateLine(index, "quantity", event.target.value)} className="h-10 rounded-lg border border-[#d0d5dd] px-3 text-sm" /><input title="Rate" type="number" min="0" value={line.price} onChange={(event) => updateLine(index, "price", event.target.value)} className="h-10 rounded-lg border border-[#d0d5dd] px-3 text-sm" /><input title="Discount" type="number" min="0" value={line.discount} onChange={(event) => updateLine(index, "discount", event.target.value)} className="h-10 rounded-lg border border-[#d0d5dd] px-3 text-sm" /><div className="flex items-center text-sm font-semibold text-[#182230]">{money(Math.max(0, line.quantity * line.price - line.discount))}</div><button type="button" onClick={() => setLines((current) => current.length === 1 ? [{ ...emptyLine }] : current.filter((_, lineIndex) => lineIndex !== index))} className="text-xl text-[#c13c25]">×</button><div className="lg:col-span-6 text-xs text-[#98a2b3]">{line.name || "Choose an item"} · {line.unit}</div></div>)}</div></div>
                <label className="mt-5 block text-sm font-semibold text-[#344054]">Internal note<textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional note for this invoice" className="mt-2 min-h-24 w-full resize-none rounded-xl border border-[#d0d5dd] p-3 font-normal outline-none focus:border-[#4f46e5]" /></label></div>
              <aside className="h-fit rounded-2xl border border-[#e7e5e4] bg-[#fbfaf9] p-6"><p className="text-sm font-semibold text-[#4f46e5]">Invoice summary</p><dl className="mt-6 space-y-4 text-sm"><div className="flex justify-between text-[#667085]"><dt>Subtotal</dt><dd>{money(subtotal)}</dd></div><div className="flex justify-between text-[#667085]"><dt>Discount</dt><dd>- {money(discount)}</dd></div><div className="flex justify-between border-t border-[#e5e7eb] pt-4 text-lg font-bold text-[#182230]"><dt>Total</dt><dd>{money(total)}</dd></div></dl><label className="mt-6 block text-sm font-semibold text-[#344054]">Payment method<select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-[#d0d5dd] bg-white px-3 font-normal"><option>Credit</option><option>Cash</option><option>UPI</option><option>Bank</option><option>Other</option></select></label><label className="mt-4 block text-sm font-semibold text-[#344054]">Received<input type="number" min="0" max={total} value={received} onChange={(event) => setReceived(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-[#d0d5dd] px-3 font-normal" /></label><div className="mt-5 space-y-2 rounded-xl bg-white p-4 text-sm"><div className="flex justify-between"><span className="text-[#667085]">Paid now</span><strong className="text-[#168361]">{money(receivedAmount)}</strong></div><div className="flex justify-between"><span className="text-[#667085]">Balance</span><strong className="text-[#b7791f]">{money(balance)}</strong></div></div><button type="button" onClick={saveInvoice} disabled={saving || loading || !activeBusinessId} className="mt-6 w-full rounded-xl bg-[#4f46e5] py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-200 disabled:opacity-50">{saving ? "Saving invoice…" : "Save invoice"}</button></aside>
            </div>
          </section>}
        </div>
      </main>

      {selectedSale && <div className="fixed inset-0 z-50 bg-[#101828]/45 p-4" onClick={() => setSelectedSale(null)}><div className="mx-auto flex max-h-[92vh] max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}><header className="flex items-center justify-between border-b border-[#eaecf0] p-6"><div><p className="text-sm font-semibold text-[#4f46e5]">Sales invoice</p><h2 className="mt-1 text-xl font-bold text-[#182230]">{selectedSale.invoiceNumber}</h2><p className="mt-1 text-sm text-[#667085]">{selectedSale.customerName || "Walk-in customer"} · {selectedSale.invoiceDate}</p></div><button onClick={() => setSelectedSale(null)} className="rounded-lg px-3 py-2 text-2xl text-[#667085] hover:bg-[#f2f4f7]">×</button></header><div className="flex-1 overflow-y-auto p-6"><table className="w-full text-left text-sm"><thead className="bg-[#fbfaf9] text-xs uppercase text-[#667085]"><tr><th className="p-3">Item</th><th className="p-3">Qty</th><th className="p-3">Rate</th><th className="p-3">Amount</th></tr></thead><tbody className="divide-y divide-[#eaecf0]">{(selectedSale.items || []).map((line, index) => <tr key={index}><td className="p-3 font-semibold text-[#344054]">{line.name}</td><td className="p-3">{line.quantity}</td><td className="p-3">{money(Number(line.price || 0))}</td><td className="p-3">{money(Math.max(0, line.quantity * line.price - line.discount))}</td></tr>)}</tbody></table><div className="ml-auto mt-6 max-w-xs space-y-2 text-sm"><div className="flex justify-between"><span className="text-[#667085]">Subtotal</span><span>{money(Number(selectedSale.subtotal || 0))}</span></div><div className="flex justify-between"><span className="text-[#667085]">Discount</span><span>- {money(Number(selectedSale.discount || 0))}</span></div><div className="flex justify-between border-t pt-3 text-lg font-bold"><span>Total</span><span>{money(Number(selectedSale.total || 0))}</span></div><div className="flex justify-between"><span>Paid</span><span className="text-[#168361]">{money(Number(selectedSale.paid || 0))}</span></div><div className="flex justify-between"><span>Balance</span><span className="text-[#b7791f]">{money(Number(selectedSale.balance || 0))}</span></div></div></div><footer className="flex justify-end gap-3 border-t border-[#eaecf0] p-5"><button onClick={() => printInvoice(selectedSale)} className="rounded-xl bg-[#4f46e5] px-5 py-2.5 text-sm font-semibold text-white">Print invoice</button><button onClick={() => setSelectedSale(null)} className="rounded-xl border border-[#d0d5dd] px-5 py-2.5 text-sm font-semibold text-[#344054]">Close</button></footer></div></div>}
    </div>
  </AuthGate>;
}
