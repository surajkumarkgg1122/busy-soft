...
export interface CreateSaleCommand { businessId: string; financialYearId: string; date: string; userId: string; customerId?: string; taxableValue: Money; taxRate: number; cessRate?: number; intraState: boolean; mode: "credit" | "cash" | "bank"; totalCost?: Money; accountMap: SalePostingInput["accountMap"]; items: SaleItemCommand[]; narration?: string; idempotencyKey: string; documentId?: string; documentPayload?: Record<string, unknown>; permissions: AccountingPermission[]; }
...
/**
 * Browser-safe Sales command. Accounting mutations are posted only through the
 * trusted Next.js API; the browser never opens an accounting transaction.
 */
export async function postSalesInvoiceFromUi(input: {
  ...
  cessRate?: number;
  intraState: boolean;
  mode: "Credit" | "Cash" | "Bank" | "UPI" | "Card";
  ...
  const response = await fetch("/api/accounting/sales", {
    ...
    body: JSON.stringify({
      ...
      grossValue: money(input.grossValueRupees),
      discountPercent: Number(input.discountPercent ?? 0),
      discountAmount: money(input.discountAmountRupees ?? 0),
      taxRate: input.taxRate,
      cessRate: input.cessRate ?? 0,
      intraState: input.intraState,
      paymentMode: input.mode === "Credit" ? "credit" : input.mode === "Cash" ? "cash" : "bank",
      paidAmount: money(input.paidAmountRupees ?? 0),
      bankAccountId: input.bankAccountId,
      itemMovements: input.items,
      narration: input.narration,
      idempotencyKey: input.idempotencyKey ?? `sale-ui-${crypto.randomUUID()}`,
      documentId: input.documentId,
      documentPayload: { ...(input.documentPayload ?? {}), invoiceNumber: input.invoiceNumber ?? null },
    }),
  });

  let payload: any = null;
  try { payload = await response.json(); } catch { /* preserve useful HTTP error below */ }
  if (!response.ok || !payload?.success) {
    const message = typeof payload?.error === "string" ? payload.error : `Sales API failed (${response.status || "network"}).`;
    throw new ValidationError(message);
  }
  return (payload.result?.value ?? payload.result) as PostingResult;
}

// Legacy low-level helpers remain available for trusted/server callers and cancellation flows.
...
export async function createSale(deps: SalesServiceDeps, command: CreateSaleCommand): Promise<PostingResult> { if (!command.businessId || !command.financialYearId) throw new ValidationError("Business and financial year are required."); if (!command.idempotencyKey) throw new ValidationError("Idempotency key is required."); if (command.mode === "credit" && !command.customerId) throw new ValidationError("Customer is required for a credit sale."); if (!command.items.length) throw new ValidationError("At least one sale item is required."); const { executeSale } = await import("../../application/core"); const result=await executeSale(deps, {businessId:command.businessId,userId:command.userId,financialYearId:command.financialYearId,idempotencyKey:command.idempotencyKey,permissions:command.permissions},command); return result.value as PostingResult; }
