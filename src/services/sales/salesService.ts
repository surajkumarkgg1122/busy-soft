...
export interface CreateSaleCommand {
  ...
  items: SaleItemCommand[]; narration?: string; idempotencyKey: string; documentId?: string; documentPayload?: Record<string, unknown>;
}
...
export async function postSalesInvoiceFromUi(db: Firestore, input: { businessId: string; userId: string; date: string; customerId?: string; taxableValueRupees: number; taxRate: number; cessRate?: number; intraState: boolean; mode: "Credit" | "Cash" | "Bank" | "UPI" | "Card"; items: SaleItemCommand[]; narration?: string; idempotencyKey?: string; documentId?: string; invoiceNumber?: string; documentPayload?: Record<string, unknown>; }): Promise<PostingResult> {
  ...
  const idempotencyKey = input.idempotencyKey ?? `sale-ui-${crypto.randomUUID()}`; const documentId = input.documentId ?? crypto.randomUUID();
  const payload = { ...(input.documentPayload ?? {}), businessId: input.businessId, invoiceNumber: input.invoiceNumber ?? undefined, customerId: input.customerId ?? null, invoiceDate: input.date, items: input.items };
  return createSale({ accounting, transaction }, { businessId: input.businessId, financialYearId, date: input.date, userId: input.userId, customerId: input.customerId, taxableValue: money(input.taxableValueRupees), taxRate: input.taxRate, cessRate: input.cessRate, intraState: input.intraState, mode: input.mode === "Credit" ? "credit" : input.mode === "Cash" ? "cash" : "bank", items: input.items, accountMap: { party: "acct-debtors", sales: "acct-sales", cash: "acct-cash", bank: "acct-bank", outputCgst: "acct-output-cgst", outputSgst: "acct-output-sgst", outputIgst: "acct-output-igst", outputCess: "acct-output-cess", inventory: "acct-inventory", cogs: "acct-cogs" }, narration: input.narration, idempotencyKey, documentId, documentPayload: payload });
}
...
export async function cancelSalesInvoiceFromUi(db: Firestore, input: { businessId: string; userId: string; saleId: string; accountingVoucherId: string; date: string; items: SaleItemCommand[] }): Promise<PostingResult> {
  ...
  return accounting.runInTransaction(async tx => { const result = await reverseVoucher(tx, input.accountingVoucherId, input.userId, deps); return result; });
}
