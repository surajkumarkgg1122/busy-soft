...
export interface CreateSaleContext { businessId:string; userId:string; financialYearId:string; idempotencyKey:string; permissions:string[]; role?:string; }
...
function validate(input:CreateSaleInput){
  if(!input.date || !/^\d{4}-\d{2}-\d{2}$/.test(input.date)) throw new ValidationError("Sale date must be YYYY-MM-DD.");
  assertMoney(input.taxableValue,"Taxable value");
  if(input.taxRate<0 || input.taxRate>100) throw new ValidationError("Invalid tax rate.");
  if(!input.itemMovements.length) throw new ValidationError("At least one sale item is required.");
  for(const line of input.itemMovements){if(!line.itemId)throw new ValidationError("Item is required.");assertQuantity(line.quantity);if(line.unitCost!==undefined)assertMoney(line.unitCost,"Unit cost");}
}
...
export async function createSale(deps:SalesApplicationDeps,ctx:CreateSaleContext,input:CreateSaleInput){
  ...
  validate(input);
  return executeSale(deps,{...ctx},input);
}
