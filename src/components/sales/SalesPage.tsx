...
import { createSale, currentFinancialYearId, getSalesWorkspaceData } from "@/application/sales/service";
...
type Sale = { id: string; invoiceNumber?: string | number; customerName?: string | null; invoiceDate?: string; total?: number; paidAmount?: number; outstandingAmount?: number; paymentMethod?: string; status?: string; accountingVoucherNumber?: string; accountingVoucherId?: string };
...
export default function SalesPage() {
  ...
  const load = useCallback(async () => {
    if (!activeBusinessId) {
      setCustomers([]); setItems([]); setSales([]); setLoading(false); return;
    }
    setLoading(true);
    try {
  ...
  const reset = () => {
    setDate(today()); setCustomerId("");
    setInvoiceNo(String(Math.max(0, ...sales.map((sale) => Number(sale.invoiceNumber) || 0)) + 1));
    setPaymentMode("cash"); setReceived(0); setLines([{ ...emptyLine }, { ...emptyLine }]); setNote(""); setError(""); setShowForm(true);
  };
  ...
  async function save() {
    ...
    try {
      ...
      await createSale(
        ...
        { businessId: activeBusinessId, userId: firebaseAuth.currentUser.uid, financialYearId: currentFinancialYearId(date), idempotencyKey: `sale-${documentId}`, permissions: ["SALE_CREATE"] },
        {
          date,
          ...
          narration: note,
          documentId,
          documentPayload: { invoiceNumber: invoiceNo, customerName: customer?.name || null, paymentMethod: paymentMode, note, items: validLines },
        },
      );
