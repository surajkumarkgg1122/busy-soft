...
export default function SalesPage() {
  ...
  const subtotal = useMemo(
    () => validLines.reduce((sum, line) => sum + line.quantity * line.price, 0),
    [validLines],
  );
  const discount = useMemo(
    () =>
      validLines.reduce(
        (sum, line) =>
          sum + (line.quantity * line.price * line.discountPercent) / 100,
        0,
      ),
    [validLines],
  );
  ...
  const paid =
    paymentMode === "cash" || paymentMode === "bank"
      ? total
      : Math.max(0, Math.min(total, received));
  const outstanding = Math.max(0, total - paid);

  const filtered = sales.filter((sale) =>
    `${sale.invoiceNumber ?? ""} ${sale.customerName ?? "Cash Sale"} ${sale.status ?? ""}`
      .toLowerCase()
  ...
  const reset = () => {
    ...
    setCustomerId("");
    setInvoiceNo(
      String(Math.max(0, ...sales.map((sale) => Number(sale.invoiceNumber) || 0)) + 1),
    );
    setPaymentMode("cash");
    setReceived(0);
  ...
  async function save() {
    ...
    if (validLines.some((line) => line.taxRate !== taxRate)) {
      setError("Mixed GST rates are not supported in one invoice yet.");
      return;
    }
    if (total <= 0) {
    ...
    const businessState = String(
      activeBusiness?.business?.address?.state || "",
    )
      .trim()
      .toLowerCase();
    ...
    if (!businessState) {
      setError("Business state is required for GST classification.");
      return;
    }
    if (customerId && !customerState) {
      setError("Customer state is required for GST classification.");
      return;
    }
    ...
    try {
      ...
      await createSale(
        ...
        },
        {
          ...
          itemMovements: validLines.map((line) => ({
            itemId: line.itemId,
          ...
          {!showForm ? (
              ...
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-[#737d88]">
                    Sales invoices through the Application Layer + Accounting Core
                  </p>
                </div>
                {canCreate && (
                  <button
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[1000px] text-sm">
                    <thead className="bg-[#eef0f2] text-left text-xs uppercase text-[#596572]">
                      <tr>
                        {["Date", "Invoice", "Party", "Payment", "Paid", "Outstanding", "Total", "Voucher"].map(
                          (heading) => (
                        ...
                        filtered.map((sale) => (
                          <tr key={sale.id} className="border-t">
                            ...
                            <td className="px-3 py-3 text-right">
                              {money(Number(sale.paidAmount || 0) / 100)}
                            </td>
                            <td className="px-3 py-3 text-right">
                              {money(Number(sale.outstandingAmount || 0) / 100)}
                            </td>
                            <td className="px-3 py-3 text-right">
                              {money(Number(sale.total || 0) / 100)}
                            </td>
                            <td className="px-3 py-3 font-mono text-xs">
                              {sale.accountingVoucherNumber ||
                                sale.accountingVoucherId ||
                                "—"}
                            </td>
                          </tr>
                        ))
            ...
            <section>
              <div className="flex h-10 items-center border-b bg-[#eef0f2]">
                ...
                <span className="px-4 text-sm font-semibold">New Sales Invoice</span>
              </div>
                ...
                <div className="mt-8 overflow-x-auto border">
                  <table className="w-full min-w-[900px] text-sm">
                    <thead className="bg-[#f5f6f7]">
                      <tr>
                        {["#", "ITEM", "QTY", "UNIT", "PRICE", "DISC %", "TAX %", "AMOUNT"].map(
                          (heading) => (
                    ...
                    <tbody>
                      {lines.map((line, index) => (
                        <tr key={index} className="border-t">
                          ...
                          <td className="p-2">
                            {items.find((item) => item.id === line.itemId)?.unit || "NONE"}
                          </td>
                          <td className="p-1">
                            <input
                              type="number"
                          ...
                          <td className="p-1">
                            <input
                              ...
                              onChange={(e) =>
                                updateLine(index, "discountPercent", e.target.value)
                              }
                              className="h-9 w-full text-right"
                            />
                          </td>
                  ...
                  <div className="rounded-lg border bg-[#fafbfc] p-4">
                    ...
                    <label className="mt-4 block text-xs">
                      ...
                      <input
                        ...
                        value={received}
                        onChange={(e) =>
                          setReceived(Math.max(0, Number(e.target.value) || 0))
                        }
