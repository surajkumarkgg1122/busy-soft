...
export function validateChartOfAccounts(accounts: readonly Account[], businessId: string): void {
  const codes = new Set<string>(); const ids = new Set<string>();
  for (const account of accounts) { validateAccount(account); if(account.businessId!==businessId)throw new ValidationError("Chart contains an account from another business."); if(ids.has(account.id))throw new ValidationError(`Duplicate account id: ${account.id}`); if(codes.has(account.code))throw new ValidationError(`Duplicate account code: ${account.code}`); ids.add(account.id);codes.add(account.code); if(account.parentId===account.id)throw new ValidationError(`Account cannot be its own parent: ${account.name}`); }
...
export function accountTypeNormalBalance(type: AccountType): "debit"|"credit" { return type === "asset" || type === "expense" || type === "contra_revenue" ? "debit" : "credit"; }
export function isProfitAndLossAccount(type: AccountType): boolean { return type === "income" || type === "expense" || type === "contra_revenue"; }
