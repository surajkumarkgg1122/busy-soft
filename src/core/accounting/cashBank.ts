...
function validateBase(i:CashBankBase){if(!i.businessId||!i.financialYearId||!i.userId)throw new ValidationError("Business, financial year and user are required.");if(!/^\d{4}-\d{2}-\d{2}$/.test(i.date))throw new ValidationError("Transaction date must be YYYY-MM-DD.");if(i.idempotencyKey.length<16||i.idempotencyKey.length>128)throw new ValidationError("A valid idempotency key is required.");}
...
export async function createCashBankAccount(repo:AccountingRepository,input:CashBankAccountInput,deps:CashBankDeps):Promise<{accountId:string;ledgerAccountId:string;openingVoucherId?:string}>{
  ...
  openingAmount(input.openingBalance);if(input.openingBalanceType!=="debit"&&input.openingBalanceType!=="credit")throw new ValidationError("Invalid opening balance type.");
  ...
  return repo.runInTransaction(async tx=>{
    // Firestore transactions require every transaction read to happen before the first write.
    ...
    // Prepare every write using only the data already read above. No transaction reads follow this point.
    ...
    if(input.openingBalance>0){
      ...
      const rawLines:VoucherLineInput[]=input.openingBalanceType==="debit"
...
async function getScopedCashBankAccount(tx:any,accountId:string){return await tx.getBusinessDocument("bankAccounts",accountId) as Record<string,unknown>|null;}
...
export async function postCashBankEntry(repo:AccountingRepository,input:CashBankEntryInput,deps:CashBankDeps):Promise<PostingResult>{
  validateBase(input);amount(input.amount);const incoming=input.type==="deposit"||input.type==="cash_deposit";
  return repo.runInTransaction(async tx=>{
    const existing=await tx.getVoucherByIdempotencyKey(input.businessId,input.financialYearId,input.idempotencyKey);if(existing)return{voucher:existing};
    const account=await getScopedCashBankAccount(tx,input.accountId);if(!account)throw new ValidationError("Cash/bank account was not found in the active business.");if(account.status!=="active")throw new ValidationError("Cash/bank account is inactive.");if(String(account.ledgerAccountId)!==input.ledgerAccountId)throw new ValidationError("Cash/bank ledger account mismatch.");
    const contra=input.contraAccountId;if(!contra)throw new ValidationError("Counter account is required for cash/bank entries.");const contraAccount=await tx.getAccount(contra);if(!contraAccount||contraAccount.businessId!==input.businessId||!contraAccount.active)throw new ValidationError("Counter account is invalid or inactive.");
    if(input.partyId){const party=await tx.getBusinessDocument("parties",input.partyId);if(!party)throw new ValidationError("Selected party was not found in the active business.");if(String(party.businessId??input.businessId)!==input.businessId)throw new ValidationError("Selected party belongs to another business.");if(String(party.ledgerAccountId)!==contra)throw new ValidationError("Selected party is not linked to the selected counter account.");}
    const partyLine=input.partyId?{partyId:input.partyId}:{};const lines:VoucherLineInput[]=incoming?[dr(input.ledgerAccountId,input.amount),cr(contra,input.amount,partyLine)]:[dr(contra,input.amount,partyLine),cr(input.ledgerAccountId,input.amount)];
    ...
    await tx.saveAtomicDocument(atomic({id:`${result.voucher.id}:cashbank`,businessId:input.businessId,financialYearId:input.financialYearId,type:"journal",voucherId:result.voucher.id,idempotencyKey:input.idempotencyKey,date:input.date,createdBy:input.userId,createdAt:deps.clock.now(),payload:{operation:"cash_bank_entry",accountId:input.accountId,type:input.type,amount:input.amount,partyId:input.partyId??null,reference:input.reference??"",notes:input.notes??""}}));return result;
  });
}
...
export async function postCashBankTransfer(repo:AccountingRepository,input:CashBankTransferInput,deps:CashBankDeps):Promise<PostingResult>{
  validateBase(input);amount(input.amount);if(input.fromAccountId===input.toAccountId)throw new ValidationError("Source and destination accounts must be different.");
  return repo.runInTransaction(async tx=>{
    const existing=await tx.getVoucherByIdempotencyKey(input.businessId,input.financialYearId,input.idempotencyKey);if(existing)return{voucher:existing};const from=await getScopedCashBankAccount(tx,input.fromAccountId);const to=await getScopedCashBankAccount(tx,input.toAccountId);if(!from||!to)throw new ValidationError("Both cash/bank accounts must belong to the active business.");if(from.status!=="active"||to.status!=="active")throw new ValidationError("Both cash/bank accounts must be active.");if(String(from.ledgerAccountId)!==input.fromLedgerAccountId||String(to.ledgerAccountId)!==input.toLedgerAccountId)throw new ValidationError("Cash/bank ledger account mismatch.");
    const lines=[cr(input.fromLedgerAccountId,input.amount),dr(input.toLedgerAccountId,input.amount)];const result=await postIdempotentVoucher(tx,{businessId:input.businessId,financialYearId:input.financialYearId,voucherType:"CONTRA",prefix:"CT",date:input.date,narration:input.narration??`Transfer ${input.fromAccountId} to ${input.toAccountId}`,createdBy:input.userId,referenceType:"cash_bank_transfer",referenceId:`${input.fromAccountId}:${input.toAccountId}`,lines,idempotencyKey:input.idempotencyKey},deps);const now=deps.clock.now();await tx.saveBusinessDocument("bankAccounts",input.fromAccountId,{businessId:input.businessId,accountId:input.fromAccountId,lastVoucherId:result.voucher.id,lastTransactionAt:input.date,updatedAt:now});await tx.saveBusinessDocument("bankAccounts",input.toAccountId,{businessId:input.businessId,accountId:input.toAccountId,lastVoucherId:result.voucher.id,lastTransactionAt:input.date,updatedAt:now});await tx.saveAtomicDocument(atomic({id:`${result.voucher.id}:cashbanktransfer`,businessId:input.businessId,financialYearId:input.financialYearId,type:"journal",voucherId:result.voucher.id,idempotencyKey:input.idempotencyKey,date:input.date,createdBy:input.userId,createdAt:now,payload:{operation:"cash_bank_transfer",fromAccountId:input.fromAccountId,toAccountId:input.toAccountId,amount:input.amount,reference:input.reference??"",notes:input.notes??""}}));return result;
  });
}
