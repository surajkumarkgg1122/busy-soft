...
export async function createCashBankAccount(repo:AccountingRepository,input:CashBankAccountInput,deps:CashBankDeps):Promise<{accountId:string;ledgerAccountId:string}> {
  ...
  amount(input.openingBalance<0?Math.abs(input.openingBalance):input.openingBalance);
  ...
  return repo.runInTransaction(async tx=>{
    ...
    await tx.saveBusinessDocument("bankAccounts",input.accountId,{accountId:input.accountId,displayName:input.displayName.trim(),kind:input.kind,ledgerAccountId:input.ledgerAccountId,openingBalance:input.openingBalance,openingBalanceDate:input.openingBalanceDate,currentBalance:input.openingBalance,status:"active",createdBy:input.createdBy,createdAt:now,updatedAt:now});
...
export async function postCashBankEntry(repo:AccountingRepository,input:CashBankEntryInput,deps:CashBankDeps):Promise<PostingResult>{
  ...
  return repo.runInTransaction(async tx=>{
    ...
    await tx.saveBusinessDocument("bankAccounts",input.accountId,{accountId:input.accountId,lastVoucherId:result.voucher.id,lastTransactionAt:input.date,updatedAt:deps.clock.now()});
...
export async function postCashBankTransfer(repo:AccountingRepository,input:CashBankTransferInput,deps:CashBankDeps):Promise<PostingResult>{
  ...
  return repo.runInTransaction(async tx=>{
    ...
    const now=deps.clock.now();
    await tx.saveBusinessDocument("bankAccounts",input.fromAccountId,{lastVoucherId:result.voucher.id,lastTransactionAt:input.date,updatedAt:now});
    await tx.saveBusinessDocument("bankAccounts",input.toAccountId,{lastVoucherId:result.voucher.id,lastTransactionAt:input.date,updatedAt:now});
