...
export type AccountingPermission =
  ...
  | "ALLOCATION_CREATE" | "PARTY_VIEW" | "PARTY_CREATE" | "PARTY_EDIT" | "PARTY_ARCHIVE" | "REPORT_VIEW" | "FY_LOCK";
...
export function assertAuthorized(ctx: AuthorizationContext, permission: AccountingPermission): void {
  if (!ctx.userId || !ctx.businessId) throw new ValidationError("Authenticated business context is required.");
  if (!ctx.permissions.includes(permission)) throw new ValidationError(`Permission denied: ${permission}.`);
}
export function assertTrustedPostingBoundary(ctx: AuthorizationContext): void {
  if (!ctx.userId || !ctx.businessId) throw new ValidationError("Accounting commands require an authenticated trusted boundary.");
}
