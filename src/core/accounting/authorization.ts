...
export function assertAuthorized(ctx: AuthorizationContext, permission: AccountingPermission): void {
  if (!ctx.userId || !ctx.businessId) {
    throw new ValidationError("Authenticated business context is required.");
  }
...
export function assertTrustedPostingBoundary(ctx: AuthorizationContext): void {
  if (!ctx.userId || !ctx.businessId) {
    throw new ValidationError("Accounting commands require an authenticated trusted boundary.");
  }
}
