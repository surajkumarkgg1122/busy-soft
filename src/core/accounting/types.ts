export type AccountType = "asset" | "liability" | "equity" | "income" | "expense" | "contra_revenue";
...
export type StockSourceType="opening"|"purchase"|"purchase_return"|"sale"|"sale_return"|"sale_cancel"|"purchase_cancel"|"adjustment"|"transfer";
...
export type AccountingDocumentType="sale"|"purchase"|"sale_return"|"purchase_return"|"receipt"|"payment"|"contra"|"journal";
export interface AtomicAccountingDocument { id:string;businessId:string;financialYearId:string;type:AccountingDocumentType;voucherId:string;idempotencyKey:string;status:"posted";date:string;createdBy:string;createdAt:string;payload:Record<string,unknown>; }
export interface PostingResult { voucher:Voucher;lines:VoucherLine[];ledgerEntries:LedgerEntry[];stockMovements:StockMovement[]; }
export interface IdGenerator { next(prefix:string):string; } export interface Clock { now():string; }
export interface AccountingRepository { runInTransaction<T>(work:(tx:AccountingTransaction)=>Promise<T>):Promise<T>; }
export interface AccountingTransaction { getFinancialYear(id:string):Promise<FinancialYear|null>;getAccount(id:string):Promise<Account|null>;getVoucher(id:string):Promise<Voucher|null>;getVoucherLines(voucherId:string):Promise<VoucherLine[]>;getVouchersByReference(referenceType:string,referenceId:string):Promise<Voucher[]>;getVoucherByIdempotencyKey(businessId:string,financialYearId:string,idempotencyKey:string):Promise<Voucher|null>;getStockMovementsForSource(sourceId:string):Promise<StockMovement[]>;getStockMovementsForItem(itemId:string,warehouseId?:string,throughDate?:string):Promise<StockMovement[]>;getPartyAllocationsForVoucher(voucherId:string):Promise<PartyAllocation[]>;saveVoucher(voucher:Voucher):Promise<void>;saveVoucherLines(lines:VoucherLine[]):Promise<void>;saveLedgerEntries(entries:LedgerEntry[]):Promise<void>;saveStockMovements(movements:StockMovement[]):Promise<void>;savePartyAllocations(allocations:PartyAllocation[]):Promise<void>;saveReturnDocument(document:ReturnDocument):Promise<void>;saveAtomicDocument(document:AtomicAccountingDocument):Promise<void>;saveAuditEvent(event:AuditEvent):Promise<void>;allocateVoucherNumber(input:{businessId:string;financialYearId:string;voucherType:string;prefix?:string}):Promise<string>; }
