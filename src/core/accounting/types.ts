...
export interface Account {
  id: string; businessId: string; code: string; name: string; type: AccountType;
  parentId?: string | null; systemAccount: boolean; active: boolean;
  openingDebit: Money; openingCredit: Money; createdAt: string; updatedAt: string;
}
export interface VoucherLineInput {
  accountId: string; partyId?: string; description?: string; debit: Money; credit: Money;
  costCenterId?: string; itemId?: string; warehouseId?: string; taxCode?: string;
}
export interface VoucherLine extends VoucherLineInput {
  lineId: string; voucherId: string; businessId: string; lineNo: number;
}
export interface Voucher {
  id: string; businessId: string; financialYearId: string; voucherType: string; voucherNumber: string;
  date: string; status: VoucherStatus; referenceType?: string; referenceId?: string; narration?: string;
  totalDebit: Money; totalCredit: Money; createdBy: string; createdAt: string; updatedAt: string;
  cancelledAt?: string; cancelledBy?: string; reversalOfVoucherId?: string;
}
export interface LedgerEntry extends VoucherLine {
  date: string; voucherType: string; voucherNumber: string; createdAt: string;
}
export type StockDirection = "in" | "out";
export type StockSourceType = "opening" | "purchase" | "purchase_return" | "sale" | "sale_return" | "adjustment" | "transfer";
export interface StockMovement {
  id: string; businessId: string; financialYearId: string; date: string; itemId: string; warehouseId?: string;
  direction: StockDirection; quantity: number; unitCost: Money; value: Money; sourceType: StockSourceType;
  sourceId: string; createdBy: string; createdAt: string;
}
