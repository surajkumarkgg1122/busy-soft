...
describe("party features", () => {
  it("calculates due dates from payment terms", () => {
    expect(addCreditDays("2026-01-31", 30)).toBe("2026-03-02");
  });
  ...
  it("calculates bill-wise outstanding and ageing buckets", () => {
    ...
    expect(bills[0].outstanding).toBe(8000);
    expect(bills[0].bucket).toBe("1-30");
    expect(summarizeAgeing(bills, "2026-03-15").total).toBe(8000);
  });
