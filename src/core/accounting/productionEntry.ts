...
export async function postProduction(repo:AccountingRepository,input:ProductionInput,deps:TransactionDeps):Promise<PostingResult>{
  ...
  return repo.runInTransaction(async tx=>{
    ...
    const batch=calculateManufacturingCost({config,materialUnitCosts}); const batchOutput=batch.outputQuantity; const ratio=input.quantity/batchOutput;
    ...
    const voucherId=deps.ids.next("prod"); const lines:VoucherLineInput[]=[{accountId:finishedAccount,description:`Finished goods: ${String(item.name??input.itemId)}`,debit:total,credit:0,itemId:input.itemId,warehouseId:input.warehouseId},{accountId:input.materialInventoryAccountId,description:"Raw material consumed",debit:0,credit:costs.materialCost},...(overhead>0?[{accountId:wip!,description:"Labour and manufacturing overhead absorbed",debit:0,credit:overhead}]:[])];
    const result=await postIdempotentVoucher(tx,{businessId:input.businessId,financialYearId:input.financialYearId,voucherType:"PRODUCTION",prefix:"PROD",date:input.date,narration:`Production of ${String(item.name??input.itemId)}`,referenceType:"production",referenceId:voucherId,createdBy:input.userId,lines,idempotencyKey:input.idempotencyKey},deps);
    ...
    const unitCost=Math.round(total/input.quantity);const stockIn=createStockMovement({businessId:input.businessId,financialYearId:input.financialYearId,date:input.date,itemId:input.itemId,warehouseId:input.warehouseId,direction:"in",quantity:input.quantity,unitCost,sourceType:"production",sourceId:result.voucher.id,createdBy:input.userId,manufactureDate:input.date},deps.ids,deps.clock.now());if(stockIn.value!==total)throw new ValidationError("Production stock valuation rounding mismatch.");await tx.saveStockMovements([...stockOut,stockIn]);
...
export async function cancelProduction(repo:AccountingRepository,input:ProductionCancelInput,deps:TransactionDeps):Promise<PostingResult>{
  ...
  return repo.runInTransaction(async tx=>{
    ...
    for(const movement of originalStock){const current=await tx.getStockMovementsForItem(movement.itemId,movement.warehouseId);const qty=current.reduce((s,m)=>s+(m.direction==="in"?m.quantity:-m.quantity),0);if(movement.direction==="in"&&qty+1e-9<movement.quantity)throw new ValidationError(`Cannot cancel production: finished stock ${movement.itemId} has already been consumed or transferred.`);}
    const reversal=await reverseVoucher(tx,voucher.id,input.userId,deps);
    const reverseMovements=originalStock.map(m=>createStockMovement({businessId:input.businessId,financialYearId:m.financialYearId,date:deps.clock.now().slice(0,10),itemId:m.itemId,warehouseId:m.warehouseId,direction:m.direction==="in"?"out":"in",quantity:m.quantity,unitCost:m.unitCost,sourceType:m.direction==="in"?"sale_cancel":"purchase_cancel",sourceId:reversal.voucher.id,createdBy:input.userId},deps.ids,deps.clock.now()));
