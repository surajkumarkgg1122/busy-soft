...
function discount(gross:Money,pct:number,amount:Money){
  if(pct<0||pct>100)throw new ValidationError("Discount percentage must be between 0 and 100.");
  assertMoney(amount,"Discount amount");
  const byPercent=Math.round(gross*pct/100);
  const total=byPercent+amount;
  if(total>gross)throw new ValidationError("Discount cannot exceed sale value.");
  return{percentAmount:byPercent,fixedAmount:amount,total,taxable:gross-total};
}
...
export async function postSaleEntry(repo:AccountingRepository,input:SaleEntryInput,deps:TransactionDeps):Promise<PostingResult>{
  ...
  return repo.runInTransaction(async tx=>{
    ...
    if(pre)throw new ValidationError("A sale with this idempotency key is already posted.");
    ...
    const valued=[] as Array<{item:(typeof input.itemMovements)[number];allocations:ReturnType<typeof calculateOutgoingAllocations>}>;
    let totalCost=0;
    for(const item of input.itemMovements){
      const allocations=calculateOutgoingAllocations(await tx.getStockMovementsForItem(item.itemId,item.warehouseId,input.date),item.quantity,input.valuationMethod??"fifo");
    ...
    const payload={
      ...
      saleId:input.documentId??result.voucher.id,
      invoiceNumber:input.documentPayload?.invoiceNumber??result.voucher.voucherNumber,
      accountingVoucherId:result.voucher.id,
      accountingVoucherNumber:result.voucher.voucherNumber,
      grossValue:input.grossValue,
      discountPercent:input.discountPercent??0,
      discountPercentAmount:d.percentAmount,
      discountAmount:d.fixedAmount,
      totalDiscount:d.total,
