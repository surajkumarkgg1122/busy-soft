...
export interface SaleEntryInput {
  ...
  grossValue:Money; discountPercent?:number; discountAmount?:Money;
  taxRate:number; intraState:boolean; cessRate?:number;
  paymentMode:"cash"|"bank"|"credit"; paidAmount?:Money; bankAccountId?:string;
  accountMap:{party:string;cash?:string;bank?:string;sales:string;outputCgst?:string;outputSgst?:string;outputIgst?:string;outputCess?:string;inventory:string;cogs:string};
  itemMovements:Array<{itemId:string;quantity:number;warehouseId?:string}>;
  valuationMethod?:StockValuationMethod; narration?:string; idempotencyKey:string; documentId?:string; documentPayload?:Record<string,unknown>;
}
...
export async function postSaleEntry(repo:AccountingRepository,input:SaleEntryInput,deps:TransactionDeps):Promise<PostingResult>{
 ...
 if(input.paymentMode!=="credit"&&input.customerId&&((input.paidAmount??0)>=d.taxable))throw new ValidationError("A fully settled cash/bank sale should not carry a customer.");
 const paid=input.paidAmount??(input.paymentMode==="credit"?0:d.taxable);
 if(!Number.isSafeInteger(paid)||paid<0||paid>d.taxable)throw new ValidationError("Paid amount must be between zero and the invoice total.");
 const outstanding=d.taxable-paid;
